...
//    plot << createPoint(10, 20) << createTarget(targetDistance) << createWall(wall)
//         << createProjectileString(angleRads, targetDistance) << "\n";
    Gnuplot* gnuptr = &plot;
//    plots.push_back(plot);
    plots.push_back(gnuptr);
    *plots.back() << createPoint(10, 20) << createTarget(targetDistance) << createWall(wall)
                 << createProjectileString(angleRads, targetDistance) << "\n";
...
std::string Plotter::createPoint(int x, int y){
    std::ostringstream os;
    os << "set label at " << x << ", " << y << "\"\""
       << " point pointtype 7;";
    return os.str();
...
std::string Plotter::createProjectileString(float angleRads, int targetDistance){
    std::ostringstream os;
